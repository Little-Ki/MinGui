...
void MinGui::ColorPicker(const Vec2& size, Color& value, const std::string desc)
{
	...
	if (!Ctx.Pass) {
		if (Misc::Utils::IsMouseInRegion(DrawPos, size) && !Focoused && KeyOb.IsPressed(VK_LBUTTON)) {
			...
			Render::AddText(RSliderPos + Vec2{ -12, SliderSize.y / 2 }, "R", global_colors.color_text, AlignMode::LEFT_CENTER, Ctx.CurLevel + 257);
			Render::AddText(GSliderPos + Vec2{ -12, SliderSize.y / 2 }, "G", global_colors.color_text, AlignMode::LEFT_CENTER, Ctx.CurLevel + 257);
			Render::AddText(BSliderPos + Vec2{ -12, SliderSize.y / 2 }, "B", global_colors.color_text, AlignMode::LEFT_CENTER, Ctx.CurLevel + 257);
