...
namespace MinGui {
	...
	namespace Global {
		...
		namespace Render {
			...
			__forceinline extern void AddText(const Vec2& pos, const std::wstring& text, const Color& color, AlignMode mode, int level);
		}
	}
	...
	struct ProxyFuncs {
		...
		std::function<void(const std::wstring&, const Vec2&, const Color&, AlignMode)> DrawText;
		std::function<void(const std::wstring&, int&, int&)> GetTextSize;
	};
	...
	extern bool Window(const std::wstring& title, const Vec2& pos, const Vec2& size, const WindowFlags& flags);

	extern bool Button(const std::wstring& text, const Vec2& size);
	extern void ToggleButton(const std::wstring& text, const Vec2& size, bool& value);

	extern void CheckBox(const std::wstring& text, bool& value);

	extern void Lable(const std::wstring& text);
	extern bool ClickableLable(const std::wstring& text);

	extern void GroupBox(const std::wstring& title, const Vec2& size, bool center, std::function<void()> inner);

	extern void KeyBox(const Vec2& size, int& value, const std::wstring& desc = L"");

	extern void ComboBox(const std::vector<std::wstring>& items, const Vec2& size, int& value, const  std::wstring& desc = L"");
	extern void MultiComboBox(std::vector<MultiItem> items, const Vec2& size, const std::wstring& desc = L"");

	extern void ListBox(std::vector<MultiItem>& items, const Vec2& item_size, const std::wstring& desc = L"");

	extern void IntSlider(const int min, const int max, const Vec2& size, int& value, const std::wstring& desc = L"");
	extern void FloatSlider(const float min, const float max, const float step, const Vec2& size, float& value, const std::wstring& desc = L"");

	extern void TextBox(const Vec2& size, std::wstring& value, const int max_length, const std::wstring& desc = L"", const wchar_t& mask = L'\0');

	extern void ColorPicker(const Vec2& size, Color& value, const std::wstring& desc = L"");
}