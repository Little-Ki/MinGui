...
void MinGui::IntSlider(const int min, const int max, const Vec2& size, int& value, const std::wstring& desc)
{
	...
	std::wstring	DisplayValue = std::to_wstring(value);

	Render::AddText(DrawPos + Vec2{ 4, size.y / 2 }, L"-", global_colors.color_text_dimmer, AlignMode::CENTER, Ctx.CurLevel);
	Render::AddText(DrawPos + Vec2{ size.x - 4,size.y / 2 }, L"+", global_colors.color_text_dimmer, AlignMode::CENTER, Ctx.CurLevel);
...
void MinGui::FloatSlider(const float min, const float max, const float step, const Vec2& size, float& value, const std::wstring& desc)
{
	...
	std::wstring DisplayValue = ss.str();
